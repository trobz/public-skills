...
- Folds a group that shows up on both the removed and added side into a single "Same group, not an access change" entry instead of showing it as one group lost + one unrelated group gained. This happens two ways: if both sides embed the same xml_id, that's the exact same group regardless of what its display text says on either side - two live instances (or two versions of the same instance) can show different text for an identical, unrenamed group, e.g. an `ir.module.category` name that only differs in casing ("Point Of Sale" vs "Point of Sale") between them, and it has nothing to do with Odoo actually renaming anything. This case needs **no `--known-list` at all** - it only compares xml_ids the CSV already has. A real, version-tagged rename Odoo itself made (sourced from `resolve_renames.py`'s OpenUpgrade data) is the other case, and does need `--known-list` to look up. The `--known-list` file(s) are also where a discovered casing-drift case can be *remembered* for next time - `access-rights-groups`' `record_display_variants.py` reads the same diff CSV and writes it into `also_category`, which purpose/removed-badge matching here reads the same way it already reads `also_named`.
